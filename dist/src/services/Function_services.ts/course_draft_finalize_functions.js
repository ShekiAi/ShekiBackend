...
exports.finalizeDraftToCourse = finalizeDraftToCourse;
...
const course_draft_session_functions_1 = require("./course_draft_session_functions");
/**
 ...
 * this feature).
 */
async function finalizeDraftToCourse(sessionId, tutorId) {
    ...
    if (session.status !== "AWAITING_APPROVAL") {
        throw new Error(`Session is "${session.status}", not "AWAITING_APPROVAL" — the assistant needs to confirm the draft is ready before it can be finalized`);
    }
    const draft = session.draftJson;
    const problems = (0, course_draft_session_functions_1.checkDraftCompleteness)(draft);
    if (problems.length > 0) {
        throw new Error(`Draft is incomplete, can't create the course yet: ${problems.join("; ")}`);
    }
    const now = new Date();
    (0, progressEmitter_1.emitProgress)(sessionId, "saving", {});
    const courseId = await db_1.prisma.$transaction(async (tx) => {
        const course = await tx.course.create({
            data: {
                ...
                status: "DRAFT",
                aiGenerated: true,
                pendingMaterials: draft.materials,
                updatedAt: now,
            },
        });
        for (let i = 0; i < draft.modules.length; i++) {
            ...
            await tx.module.create({
                data: {
                    ...
                    order: m.order ?? 0,
                    courseId: course.id,
                    pendingLessons: m.lessons,
                    updatedAt: now,
                },
            });
            (0, progressEmitter_1.emitProgress)(sessionId, "module_saved", { module_title: m.module_title, index: i });
        }
    ...
    (0, progressEmitter_1.emitProgress)(sessionId, "finalized", { courseId });
    return courseId;
}
//# sourceMappingURL=course_draft_finalize_functions.js.map